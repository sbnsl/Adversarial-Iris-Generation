...
save_dir = '/home/IrisCode/BIOMDATA_chk'
...
def create_model(examples,isTraining):
    ...
    inputs=tf.concat([examples.images_1,examples.images_2],3)
    ...
    with tf.name_scope("generator_loss"):
        ...
        # Ot=tf.floor(1.0+outputs)
        gen_rec=tf.reduce_mean(tf.abs((targets) - Ot))
        # M=examples.images_2
        # M=tf.concat([M,M,M,M,M,M],1)
        #
        # g=tf.multiply((tf.abs((targets) - Ot)),M)


        O=outputs
        outputs=tf.concat([O[:,:,:,0],O[:,:,:,1],O[:,:,:,2],O[:,:,:,3],O[:,:,:,4],O[:,:,:,5]],1)
        # Ot=tf.floor(1+outputs)
